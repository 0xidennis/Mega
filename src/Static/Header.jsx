...
const Header = () => {
  ...
  return (
    ...
    <div className="container mx-auto px-4 py-4 flex items-center justify-between">
      ...
      {/* Desktop Navigation */}
      <nav className="hidden lg:flex items-center space-x-6 lg:mr-12">
        {["Home", "Loans", "About"].map((item) => (
          <button
            key={item}
            onClick={() => setActive(item)}
            className={`relative px-3 py-2 text-lg font-medium transition-colors ${
              active === item ? "bg-[#EBEBEB] text-[#3BB3C3]" : "text-[#3BB3C3]"
            }`}
          >
            {item}
            {active === item && (
    ...
    {isMenuOpen && (
      <div className="fixed inset-0 bg-white shadow-lg z-50 flex flex-col items-center py-10 space-y-6">
        ...
        <button className="px-6 py-3 text-white bg-[#3BB3C3] rounded-lg shadow hover:bg-teal-600">
          Get Started
        </button>
      </div>
    )}
